...
type TokenSnapshot struct {
	...
	// When real depth is available (raydium_wsol_vault) this is the WSOL reserve balance.
	...
	// >= 0 = verified depth from a Raydium WSOL vault getTokenAccountBalance query.
	...
	// "raydium_wsol_vault"   — on-chain WSOL reserve via verified vault getTokenAccountBalance.
	LiquidityEvidenceSource string `json:"liquidity_evidence_source"`

	// LiquidityProxyReliable is true only when LiquidityEvidenceSource is "raydium_wsol_vault".
