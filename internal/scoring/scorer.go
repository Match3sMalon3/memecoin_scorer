...
func isTradeable(f model.TokenFeatures, t config.Thresholds) bool {
	return f.CohortBuyerCount >= t.MinCohortBuyers &&
		f.ManipulationRiskScore <= t.MaxManipulationRiskScore &&
...
func isClean(f model.TokenFeatures, t config.Thresholds) bool {
	return f.MedianRealizedReturnPct >= t.MinRealizedReturnForClean &&
		f.WinnerExitRatio >= t.MinWinnerRatioForClean
}

// opportunityComponent scores buyer depth and MFE strength (0–100).
func opportunityComponent(f model.TokenFeatures, t config.Thresholds) float64 {
	...
	// MFE strength: excess above threshold, saturates at 3× the threshold.
...
func adversarialComponent(f model.TokenFeatures, t config.Thresholds) float64 {
	// Sniper intensity: 0 when at/below threshold, 100 when at double the threshold.
	var sniperRisk float64
	if t.MaxSniperIntensityRatio > 0 {
		sniperRisk = clamp(f.SniperIntensityRatio/t.MaxSniperIntensityRatio, 0, 1)
	}

	// First-minute share: 0 when at/below threshold, 100 at double.
	var fmsRisk float64
	if t.MaxFirstMinuteShare > 0 {
		fmsRisk = clamp(f.FirstMinuteShare/t.MaxFirstMinuteShare, 0, 1)
	}

	// Manipulation risk score is binary (0 or 1 in the dataset).
	manipRisk := clamp(float64(f.ManipulationRiskScore), 0, 1)
...
func monetizationComponent(f model.TokenFeatures, t config.Thresholds) float64 {
	...
	// Buy flow: prefer buy-heavy tokens (> 0.5 is positive, saturates at 1.0).
