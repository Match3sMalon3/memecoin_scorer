...
// trade size can be absorbed by the observed swap-flow liquidity proxy.
//
...
//   - Returns 0.0 when liquiditySOL <= 0 (no observed proxy evidence).
...
//
//	multiplier = 20 → position must be < 5 % of the proxy for full score (1.0).
//	multiplier = 10 → position must be < 10 % of the proxy for full score.
//
